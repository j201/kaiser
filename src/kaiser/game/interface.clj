...
(defn current-player
  ...
  [round]
  (cond
    (= 4 (count (:turn round))) (turn-winner (:turn round) (:trump (:bid round)))
  ...
  (cond
    ...
    :else (assoc round
                 ...
                 :turns (conj (:turns round) (assoc (:turn round)
                                                    :winner (turn-winner (:turn round) (:trump (:bid round)))
                                                    :points (points (:turn round)))))))
...
(defn end-round
  ...
  [round game]
  (let [score (map + (:score game) (score-round (:turns round) (:bid round) (:rules game)))
        no-trump-bid (or (:no-trump-bid game) (= :no-trump (:bid round)))
        winner (game-over? score (:bid round) no-trump-bid (:rules game))]
    (assoc game
           :score score
